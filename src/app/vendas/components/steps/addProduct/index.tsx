...
const extraFields = (item: CartProduct) => {
  return (
    <Grid container sx={{ marginTop: '0.5rem' }}>
      <Grid item xs={6}>
        <Label text={`V.B.: `} />
        {formatNumber(item?.precoVenda, 'currency')}
      </Grid>
      <Grid item xs={6} textAlign="right">
        <Label text={`Desc: `} />
        {formatNumber(item?.desconto, 'decimal')}%
      </Grid>
      <Grid item xs={3}>
        <Label text={`Local: `} />
        {item?.local}
      ...
      <Grid item xs={6} textAlign="right">
        <Label text={`V.L.: `} />
        {formatNumber(item?.valorLiquido * item?.quantidade, 'currency')}
      </Grid>
    </Grid>
  )
}
    ...
    <Stack className={styles.container}>
      <form className={styles.formContainer} onSubmit={handleSubmit(onSubmit, onErrorSubmit)}>
        <Grid container spacing={2}>
          ...
          <Grid item xs={6}>
            <SimpleInfo
              label="Valor bruto UN"
              value={selectedProduct?.precoVenda || 0}
              vertical={true}
              showLines={true}
            />
          </Grid>
          <Grid item xs={6}>
            <NumberFormatInput
              ...
              customOnChange={(value, field) => handleUpdateSelectedProduct(setValue, value, field)}
              control={control}
              hideValidation={true}
            />
          </Grid>
          ...
          <Grid item xs={6} mt="auto">
            <SimpleInfo label="Valor total" value={valorTotal} vertical={true} showLines={true} />
          </Grid>
        </Grid>
        <List.Item
          info={{ ...itemInfo }}
